...
namespace Hook {
    ...
    static auto __fastcall ReadGameFile(void* m_this, int edx, const char* name) -> int 
    {
    ...
    static auto CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) -> LRESULT
    {
        ...
        if (uMsg == WM_CREATE)
        {
            ...
            if (CP_UTF8 == Utils::OsCurrentCodePage) {
                ::SetWindowTextA(hWnd, reinterpret_cast<const char*>(nTitle));
            }
            else 
            {
                ::SetWindowTextW(hWnd, Utils::ConvertToUTF16(reinterpret_cast<const char*>(nTitle), CP_UTF8).c_str());
            }
    ...
    static auto WINAPI AppendMenuA(HMENU hMenu, UINT uFlags, UINT_PTR uIDNewItem, LPCSTR lpNewItem) -> BOOL
    {
        //console::fmt::write<936>("[AppendMenuA] %s\n", lpNewItem);
        return ::AppendMenuW(hMenu, uFlags, uIDNewItem, Utils::ConvertToUTF16(lpNewItem).c_str());
    }

    static auto WINAPI ModifyMenuA(HMENU hMnu, UINT uPosition, UINT uFlags, UINT_PTR uIDNewItem, LPCSTR lpNewItem) -> BOOL
    {
...
extern "C"  
{
    ...
    auto APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved) -> BOOL
    {
        if (ul_reason_for_call == DLL_PROCESS_ATTACH)
        {
            ...
            RELEASE_ONLY
            ({  // jmp 0x40C05F, nop nop ; 跳过免封包读取的逻辑

...
ipt_text: str = './com_doko_iku_text'
org_text: str = './txt'
out_text: str = './new'

def to_full_char(text: str) -> str:
    half: str = " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890%&,?;:'\"().!"
    full: str ="　俙俛俠俢俤俥俧俫俬俰俲俴俵俶俷俹俻俼俽俿倀倁倂倃倄倅倎倐們倓倕倖倗倛倝倞倠倢倣値倧倫倯倰倱倲倳倴倵倶倷倸侾俀俁係俆俇俈俉俋侽亾仌丆丠丟丗鶹鶺乮乯丏両"
...
def text_com(cn_text:  list[list[str|int, str]], old_text: list[str]) -> list[str]:
    ...
    for out in cn_text[1]:
        ...
        old_text[line] = org_slit[0].split(' ')[0] + ' ' + text + ',' + org_slit[1].strip('\n') + '\n'
    for bbacklog in cn_text[2]:
        line: int = bbacklog[0]
        text: str = bbacklog[1]
        org_slit = old_text[line].split(',', 1)
        old_text[line] = org_slit[0].split(' ')[0] + ' ' + text.strip('\n') + '\n'
